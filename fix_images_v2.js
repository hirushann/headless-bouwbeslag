...
// 2. Thumbnails
content = content.replace(
    `className={\`items-center justify-center border aspect-square rounded-md overflow-hidden flex-shrink-0 transition-all \${selectedImage === thumb.src ? 'border-blue-600 ring-2 ring-blue-100' : 'border-gray-200 hover:border-blue-400'}\`} aria-label={\`Thumbnail \${idx + 1}\`} type="button">
                    <img src={thumb.src} alt={\`Thumbnail \${idx + 1}\`} className="w-full h-full object-contain" />
                  </button>`,
    `className={\`items-center justify-center border aspect-square rounded-md overflow-hidden flex-shrink-0 transition-all relative \${selectedImage === thumb.src ? 'border-blue-600 ring-2 ring-blue-100' : 'border-gray-200 hover:border-blue-400'}\`} aria-label={\`Thumbnail \${idx + 1}\`} type="button">
                    <Image src={thumb.src} alt={\`Thumbnail \${idx + 1}\`} fill sizes="100px" className="object-contain" />
                  </button>`
);

content = content.replace(
    `className={\`items-center justify-center border rounded-md overflow-hidden flex-shrink-0 transition-all p-1 \${selectedImage === img.src ? 'border-blue-600 ring-2 ring-blue-100' : 'border-gray-200 hover:border-blue-400'}\`} 
    ...
    `className={\`relative items-center justify-center border rounded-md overflow-hidden flex-shrink-0 transition-all p-1 \${selectedImage === img.src ? 'border-blue-600 ring-2 ring-blue-100' : 'border-gray-200 hover:border-blue-400'}\`} 
                              aria-label={\`Thumbnail \${idx + 1}\`} 
                              type="button"
                              style={{ height: '80px', width: '80px' }}
                            >
                              <Image src={img.src} alt={\`Thumbnail \${idx + 1}\`} fill sizes="80px" className="object-contain p-1" />
                            </button>`
);
