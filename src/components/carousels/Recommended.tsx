...
export default function RecommendedCarousel({ products }: RecomendedCarouselProps) {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { root: trackRef.current, threshold: 0.1 }
    );
    ...
    <div className="hidden lg:block w-full py-10">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => scroll(1)} disabled={atEnd} className="inline-flex size-10 items-center justify-center rounded-full border border-[#D7DCE2] bg-white text-[#1C2530] disabled:cursor-not-allowed disabled:opacity-40 hover:bg-[#0066FF] hover:text-white" aria-label="Next">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width="20" height="20" fill="currentColor"><path d="M439.1 297.4C451.6 309.9 451.6 330.2 439.1 342.7L279.1 502.7C266.6 515.2 246.3 515.2 233.8 502.7C221.3 490.2 221.3 469.9 233.8 457.4L371.2 320L233.9 182.6C221.4 170.1 221.4 149.8 233.9 137.3C246.4 124.8 266.7 124.8 279.2 137.3L439.2 297.3z"/></svg>
          </button>
        </div>
      </div>
