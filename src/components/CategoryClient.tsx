        ...
        <div className="flex flex-col lg:flex-row gap-2 lg:gap-8">
          ...
          <main className="flex-1">
            ...
            {category?.top_description && (
                <div 
                  className="mb-6 prose prose-blue max-w-none leading-relaxed text-gray-800 category-description-style text-sm lg:text-base" 
                  dangerouslySetInnerHTML={{ __html: category.top_description }} 
                />
            )}
            ...
            {/* Category FAQs */}
            {category?.top_description_faqs && category.top_description_faqs.length > 0 && (
              <div className="flex flex-col mb-12 p-5 lg:p-6 bg-[#FFFFFF] rounded-lg mt-6">
                {category?.faq_title && (
                  <div className="mb-5">
                    <h2 className="text-[#1C2530] font-semibold text-2xl m-0">{category.faq_title}</h2>
                  </div>
                )}
                {category.top_description_faqs.map((faq: any, index: number) => (
                  <div key={index} className="collapse bg-white border-b !rounded-0 border-[#F5F5F5]">
                    <input type="radio" name="category-faq" defaultChecked={index === 0} aria-label={faq.question} />
                    <div className="collapse-title font-semibold flex items-center gap-2">
                      ...
                      <h3 className="w-[90%] lg:w-full m-0 text-base">{faq.question}</h3>
                    </div>
                    <div className="collapse-content text-sm">
                      <div className="m-0 text-gray-700 prose prose-sm max-w-none prose-p:m-0" dangerouslySetInnerHTML={{ __html: faq.answer }} />
                    </div>
                  </div>
                ))}
              </div>
