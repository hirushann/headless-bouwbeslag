...
export default function ProductPageClientV2({ 
  product, 
  taxRate = 21, 
  slug, 
  initialReviews,
  ...
  const modalMainImageScrollRef = useRef<HTMLDivElement>(null);
  const discounts = React.useMemo(() => getVolumeDiscounts(product), [product]);
  ...
  const basePricing = getProductPricing(product, { isB2B, vatRate: taxRate });
  const pricing = getProductPricing(product, {
    isB2B,
    vatRate: taxRate,
    quantity: Number(quantity) || 1,
    discountPercentage: selectedDiscount === null ? 0 : discounts[selectedDiscount]?.percentage ?? 0,
  });
  const sale = basePricing.unitExVat;
  const advised = basePricing.advisedExVat;
  const finalPrice = basePricing.finalPrice;
  const taxLabel = basePricing.taxLabel;
  const displayBasePrice = pricing.displayPrice;
  const cartBasePrice = pricing.cartPrice;
  const discountPercent = basePricing.discountPercent;
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
              ...
              {galleryImages.slice(0, 5).map((thumb, idx) => {
                ...
                return (
                  <button
                    key={idx}
                    onClick={() => {
                      // On mobile, clicking the 4th opens modal. On desktop, clicking the 5th opens modal.
                      // We can check window.innerWidth if we want to be exact, or just use CSS to hide the 5th.
                      // For the click handler, we can use a small hack or just open modal if they click any count box.
                      if ((isFourth && window.innerWidth < 1024 && galleryImages.length > 4) || (isFifth && galleryImages.length > 5)) {
                        setModalSelectedImage(thumb.src);
                        setIsGalleryModalOpen(true);
                      } else {
                        setSelectedImage(thumb.src);
                        if (mainImageScrollRef.current) {
                          mainImageScrollRef.current.scrollTo({
                            left: mainImageScrollRef.current.clientWidth * idx,
                            behavior: 'smooth'
                          });
                        }
                      }
                    }}
                    className={`relative aspect-square rounded-md overflow-hidden border transition-all items-center justify-center bg-white ${
                      isFifth ? 'hidden lg:flex' : 'flex'
                    } ${
                      isSelected ? 'border-blue-500 ring-2 ring-blue-500' : 'border-gray-200 hover:border-gray-400'
                    }`}
                  ...
                  >
                    ...
                    {isFourth && galleryImages.length > 4 && (
                      <div className="absolute inset-0 bg-black/60 flex lg:hidden items-center justify-center">
                    ...
                    {/* Desktop Only Count (on 5th item) */}
                    {isFifth && galleryImages.length > 5 && (
                      <div className="absolute inset-0 bg-black/60 hidden lg:flex items-center justify-center">
              ...
              {isGalleryModalOpen && (
                ...
                >
                  <motion.div 
                    ...
                    className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-full max-h-[90vh] flex flex-col overflow-hidden relative"
                  >
                    {/* Header */}
                    <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-white sticky top-0 z-10">
                      <h3 className="text-xl font-bold text-[#1C2530]">{productTitle}</h3>
                      <button 
                    ...
                    {/* Main Area */}
                    <div className="flex-1 overflow-y-auto p-4 lg:p-6 flex flex-col gap-6 lg:gap-8 bg-[#F5F5F5]">
                      {/* Big Image Carousel */}
                      <div className="flex-1 min-h-[350px] lg:min-h-[450px] relative">
                         <div 
                           ref={modalMainImageScrollRef}
                           className="absolute inset-0 flex overflow-x-auto snap-x snap-mandatory scrollbar-hide scroll-smooth"
                           onScroll={(e) => {
                             const container = e.currentTarget;
                             const index = Math.round(container.scrollLeft / container.clientWidth);
                             if (galleryImages[index]) {
                               setModalSelectedImage(galleryImages[index].src);
                             }
                           }}
                         >
                           {galleryImages.map((img, idx) => (
                         ...
                         {galleryImages.length > 1 && (
                            <div className="hidden lg:block">
                              <button
                                onClick={() => {
                                  if (modalMainImageScrollRef.current) {
                                    modalMainImageScrollRef.current.scrollBy({ left: -modalMainImageScrollRef.current.clientWidth, behavior: 'smooth' });
                                  }
                                }}
                                className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-full bg-white/90 shadow-lg border border-gray-100 text-gray-800 hover:bg-white transition-all z-10"
                                aria-label="Previous image"
                              >
                              ...
                              <button
                                ...
                                className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-full bg-white/90 shadow-lg border border-gray-100 text-gray-800 hover:bg-white transition-all z-10"
                                aria-label="Next image"
                              >
                      ...
                      <div className="mt-auto">
                        <p className="text-xs font-bold text-gray-400 mb-3 ml-1 uppercase tracking-widest">Product Galerij ({galleryImages.length})</p>
                        <div className="flex gap-3 overflow-x-auto pb-4 scroll-smooth scrollbar-hide">
                          {galleryImages.map((img, idx) => (
                            <button
                              key={idx}
                              onClick={() => {
                                setModalSelectedImage(img.src);
                                if (modalMainImageScrollRef.current) {
                                  modalMainImageScrollRef.current.scrollTo({
                                    left: modalMainImageScrollRef.current.clientWidth * idx,
                                    behavior: 'smooth'
                                  });
                                }
                              }}
                              className={`relative w-20 h-20 lg:w-24 lg:h-24 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all bg-white shadow-sm ${
                                (modalSelectedImage || galleryImages[0]?.src) === img.src 
                                  ? 'border-[#0066FF] ring-4 ring-[#0066FF]/10 z-10 scale-105' 
          ...
          <motion.div variants={fadeInUp} className="lg:w-1/2 flex flex-col gap-5">
            ...
            {(() => {
              ...
              const { advisedExVat: advised, unitExVat: sale, finalPrice, taxLabel, discountPercent } = basePricing;
              ...
              return (
                <div className="flex justify-start items-center gap-4">
                  {finalPrice !== null && finalPrice !== undefined ? (
                    <>
                      <div className="flex items-baseline gap-1.5">
                        <div className='flex flex-col lg:flex-row items-baseline gap-1.5'>
                          <span className="text-xl md:text-2xl lg:text-3xl font-bold text-[#0050D1]">
                    ...
                    basePricing.advisedDisplay !== null ? (
                      <span className="text-2xl lg:text-3xl font-bold text-[#0050D1]">
                        {formatPrice(basePricing.advisedDisplay)}
                      </span>
                    ) : (
                      <span className="text-2xl lg:text-3xl font-bold text-[#0050D1]">
                        Price not available
            ...
            <div>
              ...
              <div className='flex gap-1.5 lg:gap-3 items-center justify-center'>
                <a href={`mailto:contact@bouwbeslag.nl?subject=${encodeURIComponent(productTitle)}`} className='border border-[#0066FF] rounded-sm py-2.5 bg-white text-[#0050D1] font-bold text-sm flex items-center justify-center gap-1.5 lg:gap-3 w-full cursor-pointer hover:text-white hover:bg-[#0066FF] transition-colors'>
                ...
                <a href="tel:0031578760508" className='border border-[#0066FF] rounded-sm py-2.5 bg-white text-[#0050D1] font-bold text-sm flex items-center justify-center gap-1.5 lg:gap-3 w-full cursor-pointer hover:text-white hover:bg-[#0066FF] transition-colors'>
                    ...
                    <div className="px-6 pb-4 text-gray-700 space-y-4">
                      <div className='mt-1 flex flex-col gap-3'>
                        <p className='text-[#3D4752] font-normal text-base'>
                          ...
                          <a
                            className='text-[#0050D1] font-bold mx-1.5' rel='nofollow'
